          ...
          {api.slice(0, 6).map((item) => (
            <div className="my-5">
              <Link to={`/news/${item.id}`}>
                <div className="border-blue-700 border-b-0 border-l-0 border-r-0 border-t-2 w-75 h-40 bg-blue-50 py-5 hover:scale-105 duration-600 rounded-lg hover:shadow-lg">
                  <h1 className="font-suhayb mx-5">{item.title}</h1>
                  <div className="flex items-center my-3">
                    <img
                      src={item.image}
                      alt={item.title}
                      className="rounded-full w-12 h-12 items-center mx-5"
                    />
